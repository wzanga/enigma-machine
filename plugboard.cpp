...
//[pb.0.1] base constructor
Plugboard::Plugboard(string cfl):Component(cfl){}

//[pb.0.2] base destructor
Plugboard::~Plugboard(){}

///[pb.1.0] check that the configuration file is well-formed
int Plugboard::check_configuration(){
  ...
  if (file.fail()){
	...
	if(nbr_param!=0 && nbr_param%2!=0){
		...
		std::cerr << "Incorrect number of parameters in plugboard file "<<location<< '\n';
		file.close();
  ...
  for(file>>ws;!file.eof();file>>ws) {
    ...
    int numeric = alternative_stoi(str_nbr);
		//verification 1 : plugboard connection
    if (in_vector(table,numeric)){
      std::cerr << "Impossible configuration in plugboard file "<<location<< '\n';
      ...
      return IMPOSSIBLE_PLUGBOARD_CONFIGURATION;
		...
		if (!has_only_numeric_characters(str_nbr)){
			std::cerr << "Non-numeric character in plugboard file "<<location<< '\n';
			//std::cout << "causing exit:"<<str_nbr<< '\n';
			file.close();
		...
		if (numeric<0 || numeric >25){
		 	std::cerr << "Invalid index in plugboard file "<<location<< '\n';
			//std::cout << "causing exit:"<<str_nbr<< '\n';
			file.close();
